...
## FIXED
- A1 (CRITICAL) admin /orders/new discount decimals -- appliedDiscount PERCENTAGE computed with .toFixed(4); Shopify rejects >2 decimals, so EVERY discounted (B2B-priced) manual order returned 400 ("Applied discount value can have at most 2 digits after decimal point"). Manual B2B order creation was fully broken. Fix: .toFixed(4) -> .toFixed(2). Verified: #37073 created, line shows orig $83.99 / discounted $42.01. STATUS: fixed in working tree (branch apiwatch/entry-783); NOT yet deployed to prod admin (needs merge-to-main + restart fww-b2b-admin on :8794).
