...
await test('dashboard and spend live paths no longer use silent first-page prefixes', () => {
  assert.match(src, /drainDashboardOrders\(/);
  assert.match(src, /drainLowStockItems\(/);
  assert.match(src, /drainCustomerSpendOrders\(/);
});

await test('getOrderDetail keeps #45 shipping/fulfillment fields while draining lines', () => {
  assert.match(src, /unfulfilledQuantity/);
  assert.match(src, /currentShippingPriceSet\{presentmentMoney\{amount currencyCode\}\}/);
  assert.match(src, /fulfillmentLineItems\(first:250\)\{nodes\{quantity lineItem\{id title sku\}\}\}/);
});

await test('#45 fulfill path already pages fulfillment orders instead of a silent first:10 prefix', () => {
  // origin/main PR #45 uses getOpenFulfillmentOrderLines (multi-location arrays, no clamp).
  // Replacing it with loadOpenFulfillmentLineMap would regress that contract.
  assert.match(src, /async function getOpenFulfillmentOrderLines/);
  assert.match(src, /fulfillmentOrders\(first:10,after:\$after\)\{pageInfo\{hasNextPage endCursor\}/);
