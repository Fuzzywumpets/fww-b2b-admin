...
await test('#45 fulfill path pages both fulfillment-order connections fail-closed', () => {
  assert.match(src, /async function getOpenFulfillmentOrderLines/);
  assert.match(src, /const fos = await getOpenFulfillmentOrderLines\(orderId\)/);
  assert.match(src, /fulfillmentOrders\(first:10,after:\$after\)\{pageInfo\{hasNextPage endCursor\}/);
  assert.match(src, /nextFulfillmentCursor/);
