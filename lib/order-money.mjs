...
/**
 * Stage the same manual percentage discount on many calculated line items using bounded parallel
 * requests. Shopify executes aliased top-level mutation fields serially, so large alias batches can
 * still exceed a request timeout; independent requests against the same OPEN calculated order run
 * concurrently and remain uncommitted until the caller explicitly commits the edit.
 *
 * DEPENDS: server.mjs stageOrderDiscount uses this inside an OPEN orderEdit session and commits
 * only after every request succeeds. This helper deliberately waits for every worker before it
 * throws: no mutation may still be in flight when the caller abandons the calculated order. Timed-
 * out requests are never retried inside the session because their outcome is unknown.
 */
export async function addLineDiscountsConcurrent({ shopifyFetch, calcId, lines, discount, concurrency = 24, fetchTimeoutMs = 60000 }) {
  const all = Array.isArray(lines) ? lines : [];
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 32) {
    throw new Error('discount concurrency must be an integer from 1 to 32');
  }
  let next = 0;
  const failures = [];
  async function worker() {
    while (true) {
      ...
      try {
        // DEPENDS: the production adapter accepts a third {timeoutMs} argument. Ordinary bridge
        // calls retain 15s; only this bounded large-order operation gets the longer limit.
        const result = await shopifyFetch(
          `mutation addDiscount($id:ID!,$li:ID!,$d:OrderEditAppliedDiscountInput!){
            orderEditAddLineItemDiscount(id:$id,lineItemId:$li,discount:$d){userErrors{field message}}
          }`,
          { id: calcId, li: line.id, d: discount },
          { timeoutMs: fetchTimeoutMs },
        );
        const payload = result?.data?.orderEditAddLineItemDiscount;
        if (!payload || !Array.isArray(payload.userErrors)) {
          throw new Error('Shopify returned no discount mutation result');
        }
        for (const err of payload.userErrors) {
          failures.push({ index, message: `"${line.title || line.id}": ${err.message}` });
        }
      } catch (err) {
        failures.push({ index, message: `"${line.title || line.id}": ${err?.message || String(err)}` });
      }
    }
  }
