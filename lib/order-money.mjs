...
/**
 * Stage the same manual percentage discount on many calculated line items without one HTTP
 * round-trip per line. Shopify executes top-level mutation fields serially, while aliases let a
 * bounded group share one GraphQL request.
 *
 * DEPENDS: server.mjs stageOrderDiscount uses this inside an OPEN orderEdit session and commits
 * only after every batch succeeds. A thrown userError must therefore abort the whole edit; callers
 * must never catch-and-continue or a large order could be only partially discounted.
 */
export async function addLineDiscountsBatched({ shopifyFetch, calcId, lines, discount, batchSize = 20, fetchTimeoutMs = 15000 }) {
  const all = Array.isArray(lines) ? lines : [];
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 50) {
    throw new Error('discount batchSize must be an integer from 1 to 50');
  }
  let applied = 0;
  for (let offset = 0; offset < all.length; offset += batchSize) {
    const batch = all.slice(offset, offset + batchSize);
    const defs = ['$id:ID!', '$d:OrderEditAppliedDiscountInput!'];
    const fields = [];
    const variables = { id: calcId, d: discount };
    batch.forEach((line, i) => {
      defs.push(`$li${i}:ID!`);
      variables[`li${i}`] = line.id;
      fields.push(`d${i}:orderEditAddLineItemDiscount(id:$id,lineItemId:$li${i},discount:$d){userErrors{field message}}`);
    });
    // DEPENDS: the production adapter accepts a third {timeoutMs} argument. Ordinary bridge calls
    // retain 15s; only this bounded large-order operation gets the caller's longer per-batch limit.
    const result = await shopifyFetch(
      `mutation addDiscountBatch(${defs.join(',')}){${fields.join('\n')}}`,
      variables,
      { timeoutMs: fetchTimeoutMs },
    );
    const errors = [];
    batch.forEach((line, i) => {
      for (const err of (result?.data?.[`d${i}`]?.userErrors || [])) {
        errors.push(`"${line.title || line.id}": ${err.message}`);
      }
    });
