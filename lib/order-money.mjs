 ...
 */
export async function addLineDiscountsBatched({ shopifyFetch, calcId, lines, discount, batchSize = 20 }) {
  ...
  for (let offset = 0; offset < all.length; offset += batchSize) {
    ...
    const result = await shopifyFetch(`mutation addDiscountBatch(${defs.join(',')}){${fields.join('\n')}}`, variables);
    const errors = [];
    batch.forEach((line, i) => {
